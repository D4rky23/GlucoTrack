...
export default function Navbar() {
  ...
  useEffect(() => {
    const onScroll = () => {
      setIsScrolledDown(window.scrollY > 30)
    }
  ...
  return (
    <nav
      className={`fixed left-0 w-full z-50 h-16 flex items-center transition-all duration-300 ease-in-out
        ${isScrolledDown
          ? 'top-4 rounded-l-xl rounded-r-full shadow-lg bg-white/30 dark:bg-slate-800/30 backdrop-blur-md'
          : 'top-0 rounded-none shadow-none bg-white dark:bg-slate-800'}
        opacity-100`}
      style={{clipPath: isScrolledDown ? 'inset(0px 0px 0px 0px round 2rem 4rem 2rem 2rem)' : undefined}}
    >
